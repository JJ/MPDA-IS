...
al profesor, compañeros o buscar recursos en Internet para los mismos
(validándolos con el profesor, si es necesario).
...
* Edición del perfil en GitHub para que aparezca nombre completo y ciudad, así
   imagen del avatar que no sea la imagen por omisión. Lo primero es opcional, y
   se trata simplemente de que sea fácil identificar a la persona que está
   matriculada en la materia como propietaria del perfil de GitHub
   correspondiente. Si se desea, se puede borrar cuando se pase este objetivo.
...
3. Desde esa rama, se creará un [*pull
   ...
   *al propio repositorio* indicando los cambios realizados. Este PR tendrá que
   ser aprobado por el profesor para que se considere el objetivo
   alcanzado. También se puede hacer con el *plugin* (en parte), escribiendo
   `git iv sube-objetivo`. El mismo git imprimirá instrucciones que se podrán
...
> cada objetivo a la principal, otro en el de la materia, desde una rama también
